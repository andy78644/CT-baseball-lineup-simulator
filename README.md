...
複製圖片功能使用瀏覽器的 Clipboard API，具有以下限制：

- ✅ **桌面瀏覽器**: Chrome, Edge, Safari, Firefox 完全支援
- ⚠️ **手機瀏覽器**: 部分支援，建議使用截圖功能
- ❌ **Telegram 內建瀏覽器**: 可能無法正常使用複製功能，建議：
  - 使用截圖功能手動保存
  - 或在外部瀏覽器（Chrome、Safari）中開啟
  - 點擊右上角 "⋯" → "在瀏覽器中開啟"

如遇到複製失敗，系統會顯示錯誤提示，請改用截圖方式分享。
